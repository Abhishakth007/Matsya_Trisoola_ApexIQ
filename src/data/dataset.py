...
# FIXED: Made configurable instead of hardcoded TODO
NEGATIVE_VESSEL_TYPE_FACTOR = 32

# Heuristic based on qualitative PCA performance.
PSEUDO_HEADING_THRESHOLD_M = 100

# FIXED: Configurable parameters for runtime computation
def get_vessel_type_factor(config: dict = None) -> float:
    """Get vessel type factor from configuration or use default.
    
    Parameters
    ----------
    config: dict, optional
        Configuration dictionary containing vessel type factor
        
    Returns
    -------
    factor: float
        Vessel type factor for class balancing
    """
    if config and "vessel_type_balance_factor" in config:
        return config["vessel_type_balance_factor"]
    return NEGATIVE_VESSEL_TYPE_FACTOR


def is_near_bin_boundary(heading_degrees: float, bin_size: int = 16, threshold_degrees: float = 5.0) -> bool:
    """Check if heading is near a bin boundary for weighted assignment.
    ...
    threshold_degrees: float
        Threshold in degrees to consider "near" boundary
        
    Returns
    -------
    is_near: bool
        True if heading is near a bin boundary
    """
    degrees_per_bin = 360.0 / bin_size
    bin_center = (heading_degrees // degrees_per_bin) * degrees_per_bin + degrees_per_bin / 2
    distance_from_center = abs(heading_degrees - bin_center)
    return distance_from_center > (degrees_per_bin / 2 - threshold_degrees)
...
def custom_preprocess(
    ...
    # FIXED: Only assign this weight if label is near bin boundary
    heading_degrees = properties.get("Heading", 0)
    if is_near_bin_boundary(heading_degrees, buckets, threshold_degrees=5.0):
        for offset in [-1, 1]:
