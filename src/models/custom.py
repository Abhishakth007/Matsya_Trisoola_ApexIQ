...
class Model(torch.nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        if targets:
            ...
            # FIXED: Proper heading loss calculation
            # Convert probability distribution to class index for cross-entropy
            heading_targets = targets[:, 2:18]
            # Find the class with highest probability (argmax)
            heading_class_indices = torch.argmax(heading_targets, dim=1)
            loss_heading = torch.nn.functional.cross_entropy(
                input=output[:, 2:18], target=heading_class_indices
            )
            
            loss_speed = torch.mean(torch.abs(output[:, 18] - targets[:, 18]))
            
            # FIXED: Proper vessel type loss calculation
            vessel_type_targets = targets[:, 19:21]
            vessel_type_class_indices = torch.argmax(vessel_type_targets, dim=1)
            loss_type = torch.nn.functional.cross_entropy(
                input=output[:, 19:21],
                target=vessel_type_class_indices,
...
class SeparateHeadAttrModel(torch.nn.Module):
    ...
    def forward(self, images, targets=None):
        ...
        if targets:
            ...
            heading_class_indices = torch.argmax(heading_labels, dim=1)
            loss_heading = torch.nn.functional.cross_entropy(heading_bucket_preds, heading_class_indices)

            # FIXED: Proper activity type loss calculation
            activity_labels = targets[:, 19:21]
            activity_class_indices = torch.argmax(activity_labels, dim=1)
            loss_activity_type = torch.nn.functional.cross_entropy(
                activity_type, 
                activity_class_indices, 
                weight=torch.tensor([1, NEGATIVE_VESSEL_TYPE_FACTOR]).to(targets.device)
            )
