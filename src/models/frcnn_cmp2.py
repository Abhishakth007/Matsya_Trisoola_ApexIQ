...
class FasterRCNNModel(torch.nn.Module):
    def __init__(self, info):
        ...
        # FIXED: Proper channel handling for Sentinel-1 data
        # Data pipeline creates 6-channel arrays: [VH, VV, VH_overlap0, VV_overlap0, VH_overlap1, VV_overlap1]
        # Each group should be 2 channels (VH, VV)
        total_channels = len(info["Channels"])
        if total_channels == 6:
            # Sentinel-1: 6 channels = 3 groups of 2 channels each
            group_channels = 2
        elif total_channels == 2:
            # Sentinel-1 without overlaps: 2 channels = 1 group
            group_channels = 2
        elif total_channels == 3:
            # Sentinel-2: 3 channels = 1 group
            group_channels = 3
        else:
            # Fallback to user-specified value
            group_channels = options.get("GroupChannels", 2)
        
        logger.info(f"Total channels: {total_channels}, Group channels: {group_channels}")
    ...
    def forward(self, images, raw_targets=None):
        ...
        # FIXED: Proper target processing without creating fake labels
        # Only process targets that actually have vessels
        targets = None
        if raw_targets:
            targets = []
            for target in raw_targets:
                ...
                if len(target["boxes"]) > 0:
                    # Only increment labels for actual vessels (0 is background)
                    target["labels"] = target["labels"] + 1
                    targets.append(target)
                else:
                    # Skip empty targets - don't create fake labels
                    # This prevents training the model to detect false positives
                    logger.debug(f"Skipping empty target - no vessels to detect")
                    continue
