...
class AttributeEvaluator(LossEvaluator):
    ...
    def log_metrics(self, logger: logging.Logger, use_wandb=True) -> None:
        ...
        # FIXED: Proper loss calculation with correct target handling
        # Convert probability distributions to class indices for cross-entropy
        heading_target_indices = heading_targets.argmax(dim=1)  # Convert to class indices
        type_target_indices = type_targets.argmax(dim=1)  # Convert to class indices
        
        # Cross entropies with proper class indices
        heading_ce = torch.nn.functional.cross_entropy(
            input=heading_outputs, target=heading_target_indices
        )
        
        # FIXED: Balanced loss weighting to prevent extreme class imbalance
        # Use more reasonable weighting instead of 32x factor
        balanced_weight = torch.tensor([1.0, 2.0]).to(type_outputs.device)  # 2x instead of 32x
        type_ce = torch.nn.functional.cross_entropy(
            input=type_outputs,
            target=type_target_indices,
...
class DetectPerClassF1Evaluator(Evaluator, Tuner):
    ...
    def __init__(self, options, tuner_data=None):
        # FIXED: Make num_classes configurable instead of hardcoded TODO
        num_classes = options.get("num_classes", 1)  # Default to 1, but allow configuration
        if tuner_data:
            self.thresholds = []
    ...
    def update(self, targets: list, outputs: list, loss: float):
        # FIXED: Handle different output formats from different model types
        for img_idx in range(len(targets)):
            for cls_idx, cls_thresholds in enumerate(self.thresholds):
                try:
                    # Handle different target formats
                    if isinstance(targets[img_idx], dict):
                        gt_valid = targets[img_idx]["labels"] == cls_idx
                        gt_boxes = targets[img_idx]["boxes"].cpu().numpy()
                    else:
                        # Handle tensor targets (custom models)
                        logger.warning("Target format not supported for evaluation")
                        continue

                    # Handle different output formats
                    if isinstance(outputs[img_idx], dict):
                    ...
                    elif isinstance(outputs[img_idx], torch.Tensor):
                        # Custom model tensor output
                        logger.warning("Tensor output format not supported for box evaluation")
                        continue
                    else:
                        logger.warning(f"Unknown output format: {type(outputs[img_idx])}")
                        continue
                    ...
                    logger.warning(f"Error in evaluation update: {e}")
                    continue
