...
def train_loop(
    ...
    windows: list[dict]
        ...
        FIXED: Optimized to retrieve only relevant windows instead of all TODO
        Uses model configuration to pre-filter windows for better performance.
    ...
    # FIXED: Pre-filter windows based on model configuration for better performance
    def filter_relevant_windows(windows: list, model_cfg: dict) -> list:
        """Filter windows to only include those relevant for the current model configuration.
        ...
        filtered_windows: list
            List of windows relevant for the current model
        """
        filtered_windows = []
        
        # Filter by task if specified
        if "task" in model_cfg.get("Data", {}):
            task = model_cfg["Data"]["task"]
            filtered_windows = [w for w in windows if w.get("task") == task]
        
        # Filter by categories if specified
        if "categories" in model_cfg.get("Data", {}):
            categories = model_cfg["Data"]["categories"]
            filtered_windows = [w for w in filtered_windows if w.get("category") in categories]
        
        # Filter by split if specified
        if "splits" in options:
            splits = options["splits"]
            filtered_windows = [w for w in filtered_windows if w.get("split") in splits]
        
        logger.info(f"Filtered {len(windows)} total windows to {len(filtered_windows)} relevant windows")
        return filtered_windows

    # Apply window filtering
    windows = filter_relevant_windows(windows, model_cfg)
    ...
    # FIXED: Add proper model registry validation and error handling
    try:
    ...
    # FIXED: Add input validation for training data
    if not isinstance(example[0], torch.Tensor):
        raise ValueError(f"Training data must be torch.Tensor, got {type(example[0])}")
    
    # Validate data format and normalization
    if example[0].dtype != torch.float32:
        logger.warning(f"Training data dtype is {example[0].dtype}, expected float32")
    
    # Check for double normalization issues
    if example[0].max() <= 1.0 and example[0].min() >= 0.0:
        logger.info("Training data appears to be already normalized to [0,1]")
    else:
        logger.info(f"Training data range: [{example[0].min():.3f}, {example[0].max():.3f}]")
