...
class S2CloudMask:
    ...
    def get_cloud_mask(
        ...
        x_coord: int,
        y_coord: int,
        window_size: int = 512,  # FIXED: Use meaningful window size for cloud detection
        threshold: float = 0.4,
        average_over_pix: int = 22,
        dilation_size: int = 11,
        all_bands: bool = True,
    ):
        """Get cloud mask from sentinel-2 scene with proper window size.
        
        Parameters
        ----------
        x_coord: int
            Center x coordinate for cloud detection window
        y_coord: int
            Center y coordinate for cloud detection window
        window_size: int, optional
            Size of the window for cloud detection (default: 512)
        ...
        all_bands: bool, optional
            Use all bands for cloud detection
        ...
        """
        # FIXED: Use proper window size for meaningful cloud detection
        half_window = window_size // 2
        
        # Ensure window doesn't go outside image bounds
        x_start = max(0, x_coord - half_window)
        x_end = min(self.img_array.shape[0], x_coord + half_window)
        y_start = max(0, y_coord - half_window)
        y_end = min(self.img_array.shape[1], y_coord + half_window)
        
        # Extract meaningful region for cloud detection
        data = self.img_array[x_start:x_end, y_start:y_end, :]
        
        if data.size == 0:
            logger.warning(f"Empty data window at coordinates ({x_coord}, {y_coord})")
            return np.zeros((1, 1, 1))  # Return empty cloud mask
